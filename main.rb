...
def make_or_watch
	...
	if response.include?("create") || response.include?("make")
		...
		print "Let's make birds! Enter the name, species, description, and habitat one at a time.\n"
		...
		temp_bird = Bird.new(temp_name, temp_species, temp_description, temp_habitat)
		...
		fout = File.open("out/#{Bird.number_of_birds}_birds_out.yml", "w")
